...
patchApp() {
    ...
    includedPatches=$(jq '.' "$storagePath/$source-patches.json" 2>/dev/null || jq -n '[]')
    patchesArg=$(jq -n --argjson includedPatches "$includedPatches" --arg pkgName "$pkgName" '$includedPatches[] | select(.pkgName == $pkgName).includedPatches | if ((. | length) != 0) then (.[] | .) else empty end')
    java -jar "$cliSource"-cli-*.jar patch -fpw -b "$patchesSource"-patches-*.jar -m "$integrationsSource"-integrations-*.apk -o "apps/$appName-$appVer/base-$sourceName.apk" $riplibArgs
	for rvx in "${patchesArg[*]}"; do -i $rvx; done --keystore "$keystore" --custom-aapt2-binary ./aapt2 --options "$storagePath/$source-options.json" --exclusive "apps/$appName-$appVer/base.apk" 2>&1 | tee "$storagePath/patch_log.txt" | "${header[@]}" --begin 2 0 --ok-label "Continue" --cursor-off-label --programbox "Patching $appName $selectedVer.apk" -1 -1
    echo -e "\n\n\njava -jar \"$cliSource\"-cli-*.jar patch -fpw -b \"$patchesSource\"-patches-*.jar -m \"$integrationsSource\"-integrations-*.apk -o \"apps/$appName-$appVer/base-$sourceName.apk\" $riplibArgs$patchesArg --keystore \"$keystore\" --custom-aapt2-binary ./aapt2 --options \"$storagePath/$source-options.json\" --exclusive \"apps/$appName-$appVer/base.apk\" 2>&1 | tee \"$storagePath/patch_log.txt\" | \"${header[@]}\" --begin 2 0 --ok-label \"Continue\" --cursor-off-label --programbox \"Patching $appName $selectedVer.apk\" -1 -1" >>"$storagePath/patch_log.txt"
    tput civis
    sleep 1
    if [ ! -f "apps/$appName-$appVer/base-$sourceName.apk" ]; then
